...
const PACKING_TEMPLATE = {
  felles: [
    // === DOKUMENTER (sjekk i god tid) ===
    "Pass × 5 — sjekk gyldighet (min 6 mnd etter 15.07.2026 = må vare til 15.01.2027)",
    "Førerkort (Carl + Anne)",
    "Vognkort (på papir)",
    "Grønt forsikringskort fra bilforsikring",
    "Reiseforsikring-polise (papir + digital)",
    "Europeisk helsetrygdekort × 5",
    "Color Line-bekreftelser + parkeringsdekk × 2",
    "Hotellbookinger printet (Berlin, Hof) + Airbnb-koder (Bardolino, Grado)",
    "Restaurantbookinger printet (Veranda del Color, Esperidi, Tavernetta, de Toni m.fl.)",
    "Reichstag-bookingbekreftelse (hvis gjennomført)",
    "Monte Baldo + Miniatur Wunderland tidsslots",
    "Vaksinekort × 5",
    "Nødnummer-liste i lommeboken",
    "Liste over Airbnb-verter med telefonnumre",

    // === PENGER & KORT ===
    "Kontanter EUR (200–300 €)",
    "Kontanter NOK (for hjemreise og ferge)",
    "Bankkort hovedkonto + reservekort i annen lommebok",
    "Tesla-betalingskort registrert i bil-appen",
    "Plugsurfing-/A Better Routeplanner-app klar (backup for IONITY)",
    "Banktjenester: utenlandsbruk slått på (DNB)",
    "Vipps logget inn",

    // === HELSE & APOTEK ===
    "Humira-doser i kjøleboks (kjølekjede hele turen — kritisk for Carl)",
    "Ispakker frosne klar dagen før",
    "Resept på Humira på engelsk (toll-papir)",
    "Førstehjelpsskrin: plaster, kompresser, antiseptisk, mikropore, saks",
    "Paracetamol × 2 pakker, ibuprofen, antihistamin (cetirizin)",
    "Reisediaré-medisin (loperamid, oralyt)",
    "Tetanus-status sjekket",
    "Solkrem SPF 50 × 3 (sjekk utgangsdato)",
    "Etter-sol gel (aloe)",
    "Insektsmiddel (myggspray for Bardolino-kvelder)",
    "Termometer",
    "Veiviser for utenlandsk apotek-app (Yandex translate eller Google translate offline)",

    // === ELEKTRONIKK & LADING ===
    "Tesla mobil ladekabel Type 2 (i hanskerommet — backup)",
    "Tesla → CCS-adapter (hvis Carl har)",
    "Telefonladere × 5",
    "USB-C-kabler × 4",
    "Powerbank × 2",
    "EU-stikkadaptere × 3 (Schuko)",
    "Bilstrømadapter 12V → USB (bilkjøring)",
    "Multistikk for hotellrom",
    "Lader Garmin Fenix",
    "Stryd-lader",
    "Laktatmåler + strips (ekstra strips for hele turen)",
    "HRV-måler",
    "Reise-USB-stick med backupkopi av reisedokumenter",
    "Offline kart Google Maps Italia/Tyskland/Østerrike lastet ned",
    "Offline oversettelse-app (Google Translate)",

    // === KJØRING & BIL ===
    "Tesla 12V batteri sjekket (svikter ofte ved langtidsbruk)",
    "Tesla forvarming/lade-buffer satt opp",
    "Bil-rute lagt inn i Tesla-navi (alle stopp)",
    "Refleksvest × 2 i bilen (krav i Italia og Frankrike)",
    "Varseltrekant i bilen",
    "Tom plastsekk for søppel under kjøring",
    "Bilseter-puter / barneseter (hvis Elias bruker)",
    "Vindusvask + kluter",
    "Solvern foran-rute",

    // === KJØRE-SNACKS & SMÅMAT ===
    "Vannflasker × 5 (gjenbrukbare)",
    "Sjokomelk-flasker × 5 til hver kjøredag",
    "Nøtte- og rosin-blanding",
    "Frukt (epler, bananer)",
    "Knekkebrød, kjeks",
    "Tyggegummi (lette ører over Brennerpasset)",
    "Termos med kaffe",

    // === BAD & STRAND ===
    "Strandhåndklær × 5 (mikrofiber sparer plass)",
    "Strand-bag/-kasse",
    "Solbriller × 5 + reservebriller",
    "Caps/solhatt × 5",
    "Snorkel × 2 (Anne + barn)",
    "Vannskor til Elias (og Daniel hvis ønsker)",
    "Picnic-utstyr: lett kniv, brett, tallerkener × 5",
    "Vinåpner + sork-trekkert",
    "Innebygd flaskeløse for Gardasjøen",
    "Frisbee, ball, badminton (Elias-aktiviteter)",
    "SUP-festebånd / tørrsekk hvis dere planlegger SUP",

    // === KJØKKEN OG SELVKOST ===
    "Husholdnings-startpakke: salt, pepper, olivenolje (kjøpes ved innflytting Bardolino/Grado)",
    "Kaffekapsler hvis Airbnb har Nespresso",
    "Tepose-pakke (jokeren — italiensk te er dårlig)",
    "Brun papirpose for restmat på kjøreturen",

    // === KLÆR-FELLES ===
    "Sykkelhjelmer × 5 (hvis dere leier sykler — sjekk om utleier har)",
    "Jakker/genser for Alpedag i Seefeld (5–10 °C på 2 000 m)",
    "Lue + tynne hansker til en høyt parti i Seefeld",
    "Lett regnjakke × 5",
    "Vintersko / fjellsko for Carl + Daniel (alpehike)",

    // === HJEMME FØR AVREISE ===
    "Stoppet post / aviser",
    "Naboer informert om at huset er tomt",
    "Termostat skrudd ned",
    "Vanninnløp hovedstopp lukket",
    "Kjøleskap tømt for ferskvarer",
    "Søppel tatt ut",
    "Vinduer og dører sjekket",
    "Alarmsystem aktivert",
    "Sykler/utstyr lagret innendørs",
    "Plante-vanning ordnet",
    "Slå av WiFi-router? (eller la stå pga Tesla-/hjemmeovervåking)"
  ],

  carl: [
    // Trening
    "Løpesko × 2 (race + treningssko)",
    "Løpetøy 8 sett",
    "Sokker × 8 (anti-blemme)",
    "Løpevest med to flasker",
    "Pannelampe (for kveldsløp Bardolino)",
    "Refleksvest",
    "Garmin Fenix",
    "Stryd løpseffektmåler",
    "Laktatmåler + strips",
    "HRV-måler (Whoop / annen)",
    "Klistrebånd til vorter / blemmer",
    "Anti-chafing krem (vaseline/Bodyglide)",
    "Magnesium-tilskudd",
    // Helse Carl
    "Humira-doser (i felles kjøleboks)",
    "Resept Humira engelsk (toll)",
    "Bryllups-/familiens medikamentdokumentasjon",
    // Klær Carl
    "T-skjorter × 10",
    "Shorts × 5",
    "Bukser × 2 (1 lang for kjøledager)",
    "Skjorte for fine dining (Veranda del Color)",
    "Boksershorts × 10",
    "Badebukse × 2",
    "Pene sko (1 par)",
    "Sandaler (Birkenstock eller lignende)",
    // Personlig Carl
    "Bok / Kindle",
    "Notatbok + penn",
    "Hodetelefoner (Airpods for kjøring + løping)"
  ],

  anne: [
    "Løpesko",
    "Treningstøy 5 sett",
    "Klær: T-skjorter, kjoler, shorts",
    "Skjorter / topper × 5",
    "Sommerkjoler × 3 (en fin for Veranda del Color)",
    "Bukser × 2",
    "Boksershorts/truser × 10",
    "BH × 3 (+ 2 sports-BH)",
    "Badedrakt × 2",
    "Pareo / strandkjole",
    "Pene sko + sandaler",
    "Solbriller (reserve)",
    "Toalettmappe (sjampo, balsam, krem, ansiktskrem)",
    "Sminkemappe",
    "Hårtørker (eller sjekk om Airbnb har)",
    "Hårbørste",
    "Snorkel/finner",
    "Bok × 2-3",
    "Hodetelefoner",
    "Egne medisiner (Anne — hva som er aktuelt)",
    "Søvn-/avslapningsstøtte hvis nødvendig"
  ],

  miriam: [
    "Telefon + lader + powerbank",
    "Hodetelefoner",
    "Bok × 2-3 (sommerlesing)",
    "Tannregulering (hvis aktuelt)",
    "Sminkemateriell",
    "Hårelastikker",
    "T-skjorter × 8",
    "Shorts × 4",
    ...
    "Badedrakt × 2",
    "Sommer-pyjamas × 3",
    "Sandaler + ett par lukkede sko",
    "Strandhåndkle (eget)",
    "Strandbag",
    "Tegne-/skrive-bok",
    "Hodebånd / solhatt",
    "Saksesett til negler",
    "Egen toalettmappe"
  ],
  ...
  daniel: [
    "Telefon + lader",
    "Hodetelefoner",
    "Switch + spill (6–8 stk) + dock + lader",
    "Bok / tegneserie",
    "T-skjorter × 8",
    "Shorts × 4",
    "Bukser × 1",
    ...
    "Badebukse × 2",
    "Svømmebriller",
    "Sandaler + ett par sko",
    "Strandhåndkle",
    "Strandutstyr (Frisbee, dykkerbriller)",
    "Solbriller",
    "Caps",
    "Egne snacks-favoritter",
    "Egen toalettmappe (tannbørste, deo)"
  ],

  elias: [
    "Switch + lader + 5 spill",
    "Hodetelefoner",
    "Aktivitetsbok / tegne-bok",
    "Lego / småleker (én pose)",
    "Bamse / koseting til kvelden",
    "T-skjorter × 8",
    "Shorts × 5",
    "Boksershorts × 8",
    "Badebukse × 2",
    ...
    "Solhatt med snor",
    "Strandhåndkle (eget)",
    "Strandleker (spade, bøtte, dukke)",
    "Lett regnjakke",
    "Genser × 2",
    "Sandaler + lukkede sko",
    "Tannbørste + tannpasta i egen pose",
    "Snacks-pakke til kjøretur (rosiner, kjeks)",
    "Vannflaske med stempel"
  ]
};
